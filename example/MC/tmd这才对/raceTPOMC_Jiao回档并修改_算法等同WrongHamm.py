...
class SUBMINLIN(object):
    ...
    # def mutation_new(self, s, Tar, l_bound, r_bound, der=-1.0, prit=True):
    #     # 保证期望值
    #
    ...
    #     # cx = self.cost
    #     if der == -1.0:
    #         der = (1 / nn) * nn * int(self.cost.min())
    #         # print(der, 'fgufufuffufu')
    #     a = np.dot(self.cost, (1 - s))
    #     b = np.dot(self.cost, s)
    #
    #     if b == 0:  # 全0 gene
    #         # print('????????')
    ...
    #                 print('太小了')
    #
    #         if l_bound <= x.sum() <= r_bound and (x != x_ori).any():
    #         # if (x != x_ori).any():
    #             # if 1:
    #             return x
    ...
    def MyPOSS(self, B, n_slots, L, R=None, delta=5):
        ...
        self.popu_index_tuples = []  # TODO 一个list[tuples]，整个popu的每个个体对应popu_slots中的index(slot_i, 个体_i)
        ...
        init = np.zeros(n, 'int')
        poses = np.random.choice(range(n), size=B, replace=False)
        init[poses] = 1
        # print(init)
        init_f = self.FS(init)
        init_i = int(
            np.ceil((init.sum() - (B - L)) / slot_wid)  # 向上取整除法
        ) - 1
        popu_slots[init_i, 0] = init
        f_c_slots[init_i, 0] = init_f
        f_c_slots[init_i, 1] = init.sum()
        self.popu_index_tuples.append((init_i, 0))

        self.global_sum += init
        ...
        while t < T:
            ...
            x = self.mutate_to_card_tar(x, B, B - L, B + R, pm)
            y = self.mutate_to_card_tar(y, B, B - L, B + R, pm)

            # x = self.mutation_new(x, B, B-L, B+R, 1/self.n, printi)  # x突变
            # y = self.mutation_new(y, B, B-L, B+R, 1/self.n, printi)  # y突变
...
if __name__ == "__main__":
    ...
    myObject.MyPOSS(B, n_sl, 2, 2, delta=5)
