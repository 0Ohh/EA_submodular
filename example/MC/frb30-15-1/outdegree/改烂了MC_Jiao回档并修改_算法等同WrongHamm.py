    ...
    def put_into_popu(self, x, x_slot_index, f_x, cost_x, popu_slots, f_c_slots, ham_slots, popu_index_tuples, delta):
        ...
        for p in range(0, delta):  # p-> (0~5)
            ...
            if f_c_slots[x_slot_index, p, 0] < worst_f:
                ...
                worst_x_indices.append(p)
        worst_worst_indices = []
        for p in worst_x_indices:
            if f_c_slots[x_slot_index, p, 1] > worst_cost:
                # 在f最差的之中找到c最差的。。
                worst_cost = f_c_slots[x_slot_index, p, 1]
                worst_worst_indices = [p]
            elif f_c_slots[x_slot_index, p, 1] == worst_cost:
                worst_worst_indices.append(p)

        if not worst_worst_indices:
            print(99 / 0)
        ...
        for p in range(0, delta):
            ...
            if ham_p < worst_ham and p in worst_worst_indices:
                worst_ham_index = p
        ...
        worst_x_index = worst_ham_index

        if ((not x_is_added) and f_x > worst_f or
                (f_x == worst_f and cost_x < worst_cost)
        ):  # x暂未加入，故当前槽已满，但新个体fx > 最差者的f
            # x替换最差者
            x_is_added = True
