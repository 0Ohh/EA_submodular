...
class SUBMINLIN(object):
    ...
    def put_at(self, x, f_x, cost_x, slot_pp_mat, slot_ff, slot_cc, pi):
        slot_pp_mat[pi] = x
        slot_ff[pi] = f_x
        slot_cc[pi] = cost_x

    def put_into_popu(self, x, x_slot_index, f_x, cost_x, popu_slots, f_c_slots, ham_slots, delta):
        ff_list = f_c_slots[x_slot_index, :, 0]
        cc_list = f_c_slots[x_slot_index, :, 1]
        pp_mat = popu_slots[x_slot_index]
        empty_pi = None
        for pi in range(delta):
            if (pp_mat[pi] == x).all():  return  # 重复了
            if (pp_mat[pi] == 0).all():  empty_pi = pi
        if empty_pi is not None:    # 空个
            self.put_at(x, f_x, cost_x, pp_mat, ff_list, cc_list, empty_pi)
            self.popu_index_tuples.append((x_slot_index, empty_pi))
            self.global_sum += x
            return
        ...
        change_index = worst_f_worst_c_pis[
            np.random.randint(0, len(worst_f_worst_c_pis))
        ]
